...
startQuiz.addEventListener("click", async function () {
  ...
  let difficulty = difficultyOptions.value;
  let number = questionsNumber.value;
  myQuiz = new Quiez(category, difficulty, number);
  myQuestion = await myQuiz.getAllQuestion();
  quizOptions.classList.replace("d-flex", "d-none");
...
class Quiez {
  ...
  showResult() {
    return `
      ...
      >
        <h2 class="mb-0">
        ${this.score == this.number ? `congratulation` : `your is ${this.score} of ${this.number}`}      
        </h2>
        <button class="again btn btn-primary rounded-pill"><i class="bi bi-arrow-repeat"></i> Try Again</button>
      </div>
    `;
...
class Question {
  constructor(index) {
    this.index = index;
    this.question = myQuestion[index].question;
    this.correct_answer = myQuestion[index].correct_answer;
    this.difficulty = myQuestion[index].difficulty;
    this.incorrect_answers = myQuestion[index].incorrect_answers;
    this.category = myQuestion[index].category;
  ...
  nextQuestion() {
    this.index++;
    setTimeout(() => {
      if (this.index < myQuestion.length) {
        let myNewQuestion = new Question(this.index);
        myNewQuestion.display();
      }else{
        let result = myQuiz.showResult();
